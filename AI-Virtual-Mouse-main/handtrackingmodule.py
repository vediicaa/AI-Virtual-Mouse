...
class handDetector():
    def __init__(self,mode=False,maxHands=2,model_complexity=1,detectionCon=0.5,trackCon=0.5):#constructor
        self.mode=mode
        ...
        self.hands=self.mpHands.Hands(self.mode,self.maxHands,self.model_complexity,self.detectionCon,self.trackCon) #object for Hands for a particular instance
    ...
    def fingersUp(self):#checking which finger is open
        ...
        if self.Poslist[self.tipIds[0]][1] > self.Poslist[self.tipIds[0] - 1][1]:#checking x position of 4 is in right to x position of 3
            fingers.append(1)
        else:
            fingers.append(0)
    ...
    def findDistance(self, p1, p2, img, draw=True,r=15,t=3):# finding distance between two points p1 & p2
        x1, y1 = self.Poslist[p1][1],self.Poslist[p1][2]#getting x,y of p1
        x2, y2 = self.Poslist[p2][1],self.Poslist[p2][2]#getting x,y of p2
